...
import { defaultFluxStackConfig, environmentDefaults } from './schema'
...
function loadFromEnvironment(prefix = 'FLUXSTACK_'): Partial<FluxStackConfig> {
    ...
    if (envValue !== undefined && envValue !== '') {
      try {
        ...
        if (
          ...
          configPath.includes('maxAge') ||
          configPath.includes('collectInterval') ||
          configPath.includes('sampleRate')
        ) {
          targetType = 'number'
        } else if (
          configPath.includes('origins') ||
          configPath.includes('methods') ||
          configPath.includes('headers') ||
          configPath.includes('exporters') ||
          configPath.includes('plugins.enabled') ||
          configPath.includes('plugins.disabled')
        ) {
          targetType = 'array'
        } else if (
          configPath.includes('enabled') ||
          configPath.includes('credentials') ||
          configPath.includes('minify') ||
          configPath.includes('treeshake') ||
          configPath.includes('compress') ||
          configPath.includes('splitChunks') ||
          configPath.includes('bundleAnalyzer') ||
          configPath.includes('sourceMaps') ||
          configPath.includes('clean')
        ) {
          targetType = 'boolean'
        }
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (
      ...
      !ENV_MAPPINGS[key as keyof typeof ENV_MAPPINGS] &&
      value !== undefined &&
      value !== ''
    ) {
...
export function loadConfigSync(options: ConfigLoadOptions = {}): ConfigLoadResult {
  const { environment = process.env.NODE_ENV || 'development', envPrefix = 'FLUXSTACK_' } = options
 ...
 */
export function createConfigSubset(config: FluxStackConfig, paths: string[]): Record<string, any> {
