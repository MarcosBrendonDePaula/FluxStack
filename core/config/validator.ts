  ...
  validate(value: any, schema: any): ValidationResult {
    ...
    return {
      ...
      warnings: warnings.map(
        (w) => `${w.path}: ${w.message}${w.suggestion ? ` (${w.suggestion})` : ''}`,
      ),
...
class BusinessValidator {
  ...
  private validatePluginConfiguration(
    ...
    const missingEssential = essentialPlugins.filter(
      (p) => !enabled.includes(p) || disabled.includes(p),
    )
  ...
  return (config: FluxStackConfig): ValidationResult => {
    ...
    if (environment === 'production') {
      ...
      if (!config.monitoring.enabled) {
        result.warnings.push(
          'Monitoring is disabled in production - consider enabling for better observability',
        )
      }
    }
