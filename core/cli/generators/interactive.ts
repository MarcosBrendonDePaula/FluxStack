...
export const interactiveGenerateCommand: CliCommand = {
  ...
  examples: ['flux generate:interactive', 'flux gi'],
  handler: async (_args, _options, context) => {
    ...
    // Get name
    const name = await promptSystem.input(`Enter the ${selectedType} name:`, undefined, (value) => {
      if (!value.trim()) {
    ...
    if (selectedType === 'controller') {
      template = await promptSystem.select('Choose controller template:', [
      ...
      template = await promptSystem.select('Choose component template:', [
      ...
      template = await promptSystem.select('Choose service template:', [
      ...
      template = await promptSystem.select('Choose route template:', [
    ...
    const useCustomPath = await promptSystem.confirm('Do you want to specify a custom path?', false)

    let customPath: string | undefined
    if (useCustomPath) {
      customPath = await promptSystem.input('Enter custom path (relative to project root):')
    }

    // Confirm overwrite if files exist
    const force = await promptSystem.confirm('Overwrite existing files if they exist?', false)
    ...
    try {
      ...
      console.error(
        `❌ Failed to generate ${selectedType}:`,
...
async function suggestRelatedGenerations(
  ...
  switch (generatedType) {
    ...
    case 'service':
      suggestions.push({
        type: 'controller',
        description: `Generate ${name} controller to use this service`,
      })
      break
    case 'route':
      suggestions.push({
        type: 'controller',
        description: `Generate ${name} controller for route handlers`,
      })
      break
    case 'component':
    ...
    if (shouldGenerate) {
      ...
      if (generator) {
        try {
          ...
          console.error(
            `❌ Failed to generate ${suggestion.type}:`,
            error instanceof Error ? error.message : String(error),
          )
        }
      }
