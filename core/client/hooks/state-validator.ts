...
import type { HybridState, StateConflict, StateValidation } from '@/core/types/types'
...
export class StateValidator {
      ...
      switch (strategy) {
        ...
        case 'smart':
          ...
          if (conflict.property === 'lastUpdated') {
            ...
            ;(merged as any)[conflict.property] = Math.max(
  ...
  static validateState<T>(hybridState: HybridState<T>): boolean {
    const currentChecksum = StateValidator.generateChecksum(hybridState.data)
  ...
  static updateValidation<T>(
    ...
    return {
      ...hybridState,
      validation: StateValidator.createValidation(hybridState.data, source),
      status: 'synced',
    }
  }
}
