...
export const errorMiddleware = (options: ErrorMiddlewareOptions = {}) => {
  ...
  return new Elysia({ name: 'error-handler' }).onError(async ({ error, request, path, set }) => {
...
export const correlationIdMiddleware = () => {
  return new Elysia({ name: 'correlation-id' }).onRequest(({ request, set }) => {
...
export const requestContextMiddleware = () => {
  return new Elysia({ name: 'request-context' }).onRequest(({ request, set }) => {
