...
import type { FluxStack, PluginContext, ResponseContext } from '@/core/plugins/types'
...
import { AuthMiddleware, CryptoAuthService } from './server'
...
const _pluginConfig: any = cryptoAuthConfig
...
export const cryptoAuthPlugin: Plugin = {
  ...
  // CLI Commands
  commands: [makeProtectedRouteCommand],
  ...
  // @ts-expect-error - plugin property não está no tipo oficial mas é suportada
  plugin: new Elysia({ prefix: '/api/auth', tags: ['Authentication'] }).get(
    '/info',
    () => ({
    ...
    }),
    {
      detail: {
        ...
        description:
          'Returns information about the Ed25519-based cryptographic authentication system, including how it works, required headers, and usage examples',
  ...
  onServerStart: async (_context: PluginContext) => {
